...
class JCA(AbstractRecommender):
    ...
    def _create_inference(self):
        with tf.compat.v1.name_scope("inference"):
            ...
            # item component
            i_pre_mul = tf.transpose(a=tf.matmul(self.I_factor_vector, tf.transpose(a=self.input_OH_I)))
            i_pre_encoder = tf.matmul(tf.transpose(a=self.input_R_I), self.IV) + self.Ib1  # input to the hidden layer
            ...
            # final output
            self.Decoder = ((tf.transpose(a=tf.gather_nd(tf.transpose(a=self.u_decoder), self.col_idx)))
                            + tf.gather_nd(tf.transpose(a=self.I_Decoder), self.row_idx)) / 2.0
    
            pos_data = tf.gather_nd(self.Decoder, self.input_P_cor)
            neg_data = tf.gather_nd(self.Decoder, self.input_N_cor)
            
            self.pre_cost1 = tf.maximum(neg_data - pos_data + self.margin, tf.zeros(tf.shape(input=neg_data)[0]))
            
    def _create_loss(self):
        with tf.compat.v1.name_scope("loss"):
            
            cost1 = tf.reduce_sum(input_tensor=self.pre_cost1)  # prediction squared error
