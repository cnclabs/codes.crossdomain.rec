...
class CFGAN(AbstractRecommender):
    ...
    def build_graph(self):
        ...
        self.g_zr_loss = tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=tf.square(self.g_output - 0) * self.g_zr_dims, axis=1, keepdims=True))
        ...
        self.real_data = tf.compat.v1.placeholder(tf.float32, [None, self.num_items])
        real_data = tf.concat([self.condition, self.real_data], 1)

        d_fake = self.dis(fake_data)
        d_real = self.dis(real_data)

        g_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='gen')
        d_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='dis')

        # define loss & optimizer for G
        g_loss = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=d_fake, labels=tf.ones_like(d_fake)))

        g_loss = g_loss + self.reg_G * l2_loss(*g_vars)
        g_loss = g_loss + self.ZR_coefficient*self.g_zr_loss

        if self.opt_g == 'sgd':
            self.trainer_g = tf.compat.v1.train.GradientDescentOptimizer(self.lr_G).minimize(g_loss, var_list=g_vars)
        elif self.opt_g == 'adam':
            self.trainer_g = tf.compat.v1.train.AdamOptimizer(self.lr_G).minimize(g_loss, var_list=g_vars)

        # define loss & optimizer for D
        d_loss_real = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=d_real, labels=tf.ones_like(d_real)))
        d_loss_fake = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=d_fake, labels=tf.zeros_like(d_fake)))
        d_loss = d_loss_real + d_loss_fake + self.reg_D * l2_loss(*d_vars)

        if self.opt_d == 'sgd':
            self.trainer_d = tf.compat.v1.train.GradientDescentOptimizer(self.lr_D).minimize(d_loss, var_list=d_vars)
        elif self.opt_d == 'adam':
            self.trainer_d = tf.compat.v1.train.AdamOptimizer(self.lr_D).minimize(d_loss, var_list=d_vars)
    ...
    def _create_layer(self):
        ...
        for i, unit in enumerate(self.hiddenLayer_G):
            hidden_layer = tf.compat.v1.layers.Dense(unit, activation=tf.sigmoid,
                                           kernel_initializer=xavier_init, name="gen_h%d"%i)
            self.gen_layers.append(hidden_layer)

        # hidden -> output
        output_layer = tf.compat.v1.layers.Dense(self.num_items, activation=tf.identity,
        ...
        # hidden -> output
        output_layer = tf.compat.v1.layers.Dense(1, activation=tf.identity,
