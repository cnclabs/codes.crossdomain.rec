...
class DMF(AbstractRecommender):
    ...
    def _create_inference(self):
        with tf.compat.v1.name_scope("inference"):
            ...
            fen_zhi = tf.reduce_sum(input_tensor=net_u_2 * net_v_2, axis=1)
        
            norm_u = tf.reduce_sum(input_tensor=tf.square(net_u_2), axis=1)
            norm_v = tf.reduce_sum(input_tensor=tf.square(net_v_2), axis=1)
