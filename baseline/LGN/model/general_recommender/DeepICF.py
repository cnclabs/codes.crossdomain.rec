...
class DeepICF(AbstractRecommender):
    ...
    def batch_norm_layer(self, x, train_phase, scope_bn):
        ...
        z = tf.cond(pred=train_phase, true_fn=lambda: bn_train, false_fn=lambda: bn_inference)
        return z
    ...
    def _create_variables(self, params=None):
        with tf.compat.v1.name_scope("embedding"):  # The embedding initialization is unknown now
            if params is None:
            ...
            self.biases = {'out': tf.Variable(tf.random.normal([1]), name='biases_out')}
    ...
    def _attention_MLP(self, q_):
        with tf.compat.v1.name_scope("attention_MLP"):
            b = tf.shape(input=q_)[0]
            n = tf.shape(input=q_)[1]
            ...
            exp_sum = tf.reduce_sum(input_tensor=exp_A_, axis=1, keepdims=True)  # (b, 1)
            exp_sum = tf.pow(exp_sum, tf.constant(self.beta, tf.float32, [1]))

            A = tf.expand_dims(tf.compat.v1.div(exp_A_, exp_sum), 2)  # (b, n, 1)

            return A, tf.reduce_sum(input_tensor=A * self.embedding_q_, axis=1)

    def _create_inference(self):
        with tf.compat.v1.name_scope("inference"):
            self.embedding_q_ = tf.nn.embedding_lookup(params=self.embedding_Q_, ids=self.user_input)  # (b, n, e)
            self.embedding_q = tf.nn.embedding_lookup(params=self.embedding_Q, ids=self.item_input)  # (b, 1, e)

            if self.algorithm == 0:  # prod
                # (?, k)
                self.A, self.embedding_p = self._attention_MLP(self.embedding_q_ * tf.expand_dims(self.embedding_q, 1))
            else:  # concat
                n = tf.shape(input=self.user_input)[1]
                self.A, self.embedding_p = self._attention_MLP(tf.concat([self.embedding_q_,
                                                                          tf.tile(tf.expand_dims(self.embedding_q, 1),
                                                                                  tf.stack([1, n, 1]))], 2))  # (?, k)

            self.bias_i = tf.nn.embedding_lookup(params=self.bias, ids=self.item_input)
            ...
            out_layer = tf.reduce_sum(input_tensor=tf.matmul(layer1, self.weights['out']) + self.biases['out'], axis=1)  # (?, 1)
