...
class NAIS(AbstractRecommender):
    ...
    def _create_inference(self, user_input, item_input, num_idx):
        with tf.compat.v1.name_scope("inference"):
            embedding_q_ = tf.nn.embedding_lookup(params=self.embedding_Q_, ids=user_input)  # (b, n, e)
            embedding_q = tf.expand_dims(tf.nn.embedding_lookup(params=self.embedding_Q, ids=item_input), 1)  # (b, 1, e)
            
            if self.algorithm == 0:
                embedding_p = self._attention_mlp(embedding_q_ * embedding_q, embedding_q_, num_idx)
            else:
                n = tf.shape(input=user_input)[1]
                embedding_p = self._attention_mlp(tf.concat([embedding_q_, tf.tile(embedding_q, tf.stack([1, n, 1]))], 2),
                                                  embedding_q_, num_idx)

            embedding_q = tf.reduce_sum(input_tensor=embedding_q, axis=1)
            bias_i = tf.nn.embedding_lookup(params=self.bias, ids=item_input)
            coeff = tf.pow(num_idx, tf.constant(self.alpha, tf.float32, [1]))
            output = coeff * tf.reduce_sum(input_tensor=embedding_p*embedding_q, axis=1) + bias_i
    ...
    def _attention_mlp(self, q_, embedding_q_, num_idx):
            with tf.compat.v1.name_scope("attention_MLP"):
                ...
                return tf.reduce_sum(input_tensor=A * embedding_q_, axis=1)
