...
class APR(AbstractRecommender):
    ...
    def _create_inference(self, item_input):
        with tf.compat.v1.name_scope("inference"):
            # embedding look up
            self.embedding_p = tf.nn.embedding_lookup(params=self.embedding_P, ids=self.user_input)
            self.embedding_q = tf.nn.embedding_lookup(params=self.embedding_Q, ids=item_input)  # (b, embedding_size)
            return tf.reduce_sum(input_tensor=self.embedding_p * self.embedding_q,axis=1) # (b, embedding_size) * (embedding_size, 1)
    
    def _create_inference_adv(self, item_input):
        with tf.compat.v1.name_scope("inference_adv"):
            # embedding look up
            self.embedding_p = tf.nn.embedding_lookup(params=self.embedding_P, ids=self.user_input)
            self.embedding_q = tf.nn.embedding_lookup(params=self.embedding_Q, ids=item_input)  # (b, embedding_size)
            # add adversarial noise
            self.P_plus_delta = self.embedding_p + tf.nn.embedding_lookup(params=self.delta_P, ids=self.user_input)
            self.Q_plus_delta = self.embedding_q + tf.nn.embedding_lookup(params=self.delta_Q, ids=item_input)
            return tf.reduce_sum(input_tensor=self.P_plus_delta * self.Q_plus_delta,axis=1) # (b, embedding_size) * (embedding_size, 1)
    ...
    def _create_loss(self):
        with tf.compat.v1.name_scope("loss"):
            ...
            self.loss = tf.reduce_sum(input_tensor=tf.nn.softplus(-self.result))
            ...
            if self.adver:
                ...
                self.loss_adv = tf.reduce_sum(input_tensor=tf.nn.softplus(-self.result_adv))
                self.opt_loss += self.reg_adv * self.loss_adv

    def _create_adversarial(self):
        with tf.compat.v1.name_scope("adversarial"):
            # generate the adversarial weights by random method
            if self.adv == "random":
                # generation
                self.adv_P = tf.random.truncated_normal(shape=[self.num_users, self.embedding_size], mean=0.0, stddev=0.01)
                self.adv_Q = tf.random.truncated_normal(shape=[self.num_items, self.embedding_size], mean=0.0, stddev=0.01)
            ...
            # generate the adversarial weights by gradient-based method
                ...
                self.grad_P, self.grad_Q = tf.gradients(ys=self.loss, xs=[self.embedding_P, self.embedding_Q])
