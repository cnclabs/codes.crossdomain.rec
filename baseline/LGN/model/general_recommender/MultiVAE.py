...
class MultiVAE(AbstractRecommender):
    ...
    def q_graph(self):
        ...
        for i, (w, b) in enumerate(zip(self.weights_q, self.biases_q)):
            ...
            if i != len(self.weights_q) - 1:
                ...
                KL = tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=0.5 * (-logvar_q + tf.exp(logvar_q) + tf.pow(mu_q,2) - 1), axis=1))
    ...
    def _create_inference(self):
        with tf.compat.v1.name_scope("inference"):
            ...
            epsilon = tf.random.normal(tf.shape(input=std_q), mean=0, stddev=0.01)
    ...
    def _create_loss(self):
        with tf.compat.v1.name_scope("loss"):  
            neg_ll = -tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=self.log_softmax_var * self.input_ph, axis=1))
