...
class GEN(object):
    def __init__(self, item_num, user_num, emb_dim, lamda, param=None, init_delta=0.05, learning_rate=0.05):
        ...
        with tf.compat.v1.variable_scope('generator'):
            if self.param is None:
                self.user_embeddings = tf.Variable(
                    tf.random.uniform([self.userNum, self.emb_dim], minval=-self.init_delta, maxval=self.init_delta,
                                      dtype=tf.float32))
                self.item_embeddings = tf.Variable(
                    tf.random.uniform([self.itemNum, self.emb_dim], minval=-self.init_delta, maxval=self.init_delta,
                                      dtype=tf.float32))
        ...
        self.all_logits = tf.reduce_sum(input_tensor=tf.multiply(self.u_embedding, self.item_embeddings), axis=1) + self.item_bias
        self.i_prob = tf.gather(
            tf.reshape(tf.nn.softmax(tf.reshape(self.all_logits, [1, -1])), [-1]),
            self.i)

        self.gan_loss = -tf.reduce_mean(input_tensor=tf.math.log(self.i_prob) * self.reward) + \
...
class DIS(object):
    def __init__(self, item_num, user_num, emb_dim, lamda, param=None, init_delta=0.05, learning_rate=0.05):
        ...
        self.pre_logits = tf.reduce_sum(input_tensor=tf.multiply(self.u_embedding, self.i_embedding), axis=1) + self.i_bias
        ...
        # for dns sample
        self.dns_rating = tf.reduce_sum(input_tensor=tf.multiply(self.u_embedding, self.item_embeddings), axis=1) + self.item_bias
