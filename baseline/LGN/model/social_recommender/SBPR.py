...
class SBPR(SocialAbstractRecommender):
    ...
    def _create_placeholders(self):
        with tf.compat.v1.name_scope("input_data"):
            ...
            self.item_input_social = tf.compat.v1.placeholder(tf.int32, shape=[None], name="item_input_social")
            self.suk = tf.compat.v1.placeholder(tf.float32, shape=[None], name="suk")
    ...
    def _create_inference(self, item_input):
        with tf.compat.v1.name_scope("inference"):
            ...
            output = tf.reduce_sum(input_tensor=tf.multiply(user_embedding, item_embedding), axis=1) + item_bias
