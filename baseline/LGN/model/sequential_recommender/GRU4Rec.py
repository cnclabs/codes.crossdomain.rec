...
class GRU4Rec(SeqAbstractRecommender):
    ...
    def _create_variable(self):
        ...
        self.Y_ph = tf.compat.v1.placeholder(tf.int32, [self.batch_size], name='output')
    ...
    def _bpr_loss(self, logits):
        ...
        loss = tf.reduce_mean(input_tensor=log_loss(pos_logits-logits))
        return loss
    ...
    def _top1_loss(self, logits):
        ...
        pos_logits = tf.linalg.diag_part(logits)  # (b,)
        pos_logits = tf.reshape(pos_logits, shape=[-1, 1])  # (b, 1)
        loss1 = tf.reduce_mean(input_tensor=tf.sigmoid(-pos_logits + logits), axis=-1)  # (b,)
        loss2 = tf.reduce_mean(input_tensor=tf.sigmoid(tf.pow(logits, 2)), axis=-1) - \
                tf.squeeze(tf.sigmoid(tf.pow(pos_logits, 2))/self.batch_size)  # (b,)
        return tf.reduce_mean(input_tensor=loss1+loss2)
