...
def l2_distance(a, b, name="euclidean_distance"):
    return tf.norm(tensor=a - b, ord='euclidean', axis=-1, name=name)
...
class TransRec(SeqAbstractRecommender):
    ...
    def _create_inference(self, item_input):
        with tf.compat.v1.name_scope("inference"):
            ...
            batch_size = tf.shape(input=user_embedding)[0]
            item_embedding_recent = tf.nn.embedding_lookup(params=self.item_embeddings, ids=self.item_input_recent)
            item_embedding = tf.nn.embedding_lookup(params=self.item_embeddings, ids=item_input)
            
            item_bias = tf.nn.embedding_lookup(params=self.item_biases, ids=item_input)
            predict_vector = user_embedding + tf.tile(self.global_embedding, tf.stack([batch_size, 1])) + \
                             item_embedding_recent - item_embedding
            predict = item_bias-tf.reduce_sum(input_tensor=tf.square(predict_vector), axis=1)
    ...
    def build_graph(self):
        ...
        u_emb = tf.nn.embedding_lookup(params=self.user_embeddings, ids=self.user_input)
        last_i_emb = tf.nn.embedding_lookup(params=self.item_embeddings, ids=self.item_input_recent)
