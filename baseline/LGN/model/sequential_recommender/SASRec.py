...
class SASRec(SeqAbstractRecommender):
    ...
    def _create_variable(self):
        ...
        l2_regularizer = tf.keras.regularizers.l2(0.5 * (self.l2_emb))
    ...
    def build_graph(self):
        ...
        with tf.compat.v1.variable_scope("SASRec", reuse=reuse):
            ...
            self.seq = tf.nn.embedding_lookup(params=self.item_embeddings, ids=self.item_seq_ph)
            item_emb_table = self.item_embeddings

            # Positional Encoding
            position = tf.tile(tf.expand_dims(tf.range(tf.shape(input=self.item_seq_ph)[1]), 0),
                               [tf.shape(input=self.item_seq_ph)[0], 1])
            t = tf.nn.embedding_lookup(params=self.position_embeddings, ids=position)
            ...
            mask = tf.expand_dims(tf.cast(tf.not_equal(self.item_seq_ph, self.items_num), dtype=tf.float32), -1)
            self.seq *= mask
        ...
        pos = tf.reshape(self.item_pos_ph, [tf.shape(input=self.item_seq_ph)[0] * self.max_len])  # (b*l,)
        neg = tf.reshape(self.item_neg_ph, [tf.shape(input=self.item_seq_ph)[0] * self.max_len])  # (b*l,)
        pos_emb = tf.nn.embedding_lookup(params=item_emb_table, ids=pos)  # (b*l, d)
        neg_emb = tf.nn.embedding_lookup(params=item_emb_table, ids=neg)  # (b*l, d)
        seq_emb = tf.reshape(self.seq, [tf.shape(input=self.item_seq_ph)[0] * self.max_len, self.hidden_units])  # (b*l, d)
        ...
        is_target = tf.reshape(tf.cast(tf.not_equal(pos, self.items_num), dtype=tf.float32),
                               [tf.shape(input=self.item_seq_ph)[0] * self.max_len])

        pos_loss = -tf.math.log(tf.sigmoid(self.pos_logits) + 1e-24) * is_target
        neg_loss = -tf.math.log(1 - tf.sigmoid(self.neg_logits) + 1e-24) * is_target
        self.loss = tf.reduce_sum(input_tensor=pos_loss + neg_loss) / tf.reduce_sum(input_tensor=is_target)
