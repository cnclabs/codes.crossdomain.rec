...
class BiTGCF(object):
    def __init__(self, data_config,args, pretrain_data, sess):
        # argument settings
        ...
        if self.initial_type == 'x':
            ...
            self.initializer = tf.compat.v1.random_normal_initializer()
        ...
        *********************************************************
        ...
        self.users_s = tf.compat.v1.placeholder(tf.int32, shape=(None,))
        self.users_t = tf.compat.v1.placeholder(tf.int32, shape=(None,))
        self.items_s = tf.compat.v1.placeholder(tf.int32, shape=(None,))
        self.items_t = tf.compat.v1.placeholder(tf.int32, shape=(None,))
        self.label_s = tf.compat.v1.placeholder(tf.float32,shape=(None,))
        self.label_t = tf.compat.v1.placeholder(tf.float32,shape=(None,))
        ...
        *********************************************************
        ...
        """
        self.u_g_embeddings_s = tf.nn.embedding_lookup(params=self.ua_embeddings_s, ids=self.users_s)
        self.u_g_embeddings_t = tf.nn.embedding_lookup(params=self.ua_embeddings_t, ids=self.users_t)
        self.i_g_embeddings_s = tf.nn.embedding_lookup(params=self.ia_embeddings_s, ids=self.items_s)
        self.i_g_embeddings_t = tf.nn.embedding_lookup(params=self.ia_embeddings_t, ids=self.items_t)
    ...
    def s_t_la2add_layer(self,input_s,input_t,lambda_s,lambda_t,domain_laplace):
        ...
        u_g_embeddings_s_lap = tf.transpose(a=tf.add(laplace_s*tf.transpose(a=u_g_embeddings_s),laplace_t*tf.transpose(a=u_g_embeddings_t)))
    ...
    def _create_embed(self,weights_s,weights_t,norm_adj_s,norm_adj_t):
        def one_graph_layer_gcf(A_fold_hat,ego_embeddings,weights,k):
            ...
            for f in range(self.n_fold):
                # print(f"A_fold_hat[f]: {A_fold_hat[f].shape}")
                # print(f"ego_embeddings: {ego_embeddings.shape}")
                try:
                    #print(f"A_fold_hat[f]: {A_fold_hat[f].shape}")
                    #print(f"ego_embeddings: {ego_embeddings.shape}")
            ...
            # message dropout.
            ego_embeddings = tf.nn.dropout(ego_embeddings, rate=self.mess_dropout[k])
    ...
    def get_scores(self,users,pos_items):
        scores = tf.reduce_sum(input_tensor=tf.multiply(users, pos_items),axis = 1)
        return scores
    ...
    def get_embeddings(self, ):
        # get all target domain embeddings
        users = [i for i in range(self.n_users)]
        items = [j for j in range(self.n_items_t)]
        feed_dict = {self.users_t: users, self.items_t: items, \
                    self.node_dropout: eval(self.args.node_dropout),
                    self.mess_dropout: eval(self.args.mess_dropout),
                    self.isTraining:True}
        user_embeddings_t = self.sess.run(self.u_g_embeddings_t, feed_dict=feed_dict)
        item_embeddings_t = self.sess.run(self.i_g_embeddings_t, feed_dict=feed_dict)

        #print("type: ", type(user_embeddings))
        #print("user embeddings: ", user_embeddings)
        #print("item embeddings: ", item_embeddings)

        return user_embeddings_t, item_embeddings_t
