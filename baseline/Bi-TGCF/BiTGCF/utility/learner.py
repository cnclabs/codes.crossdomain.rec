...
def optimizer(learner,loss,learning_rate,momentum=0.9):
    ...
    if learner.lower() == "adagrad": 
        ...
        optimizer = tf.compat.v1.train.MomentumOptimizer(learning_rate,momentum).minimize(loss)  
    else :
...
def pairwise_loss(loss_function,y,margin=1):
    ...
    if loss_function.lower() == "bpr":
        loss = -tf.reduce_sum(input_tensor=tf.math.log_sigmoid(y))
    elif loss_function.lower() == "hinge":
        loss = tf.reduce_sum(input_tensor=tf.maximum(y+margin, 0))
    elif loss_function.lower() == "square":  
        loss = tf.reduce_sum(input_tensor=tf.square(1-y))   
    else:
...
def pointwise_loss(loss_function,y_rea,y_pre):
    ...
    if loss_function.lower() == "cross_entropy":
        loss = tf.compat.v1.losses.sigmoid_cross_entropy(y_rea,y_pre)
#         loss = - tf.reduce_sum(
#             y_rea * tf.log(y_pre) + (1 - y_rea) * tf.log(1 - y_pre)) 
    elif loss_function.lower() == "square":  
        loss = tf.reduce_sum(input_tensor=tf.square(y_rea-y_pre))  
    else:
        raise Exception("please choose a suitable loss function") 
    return loss