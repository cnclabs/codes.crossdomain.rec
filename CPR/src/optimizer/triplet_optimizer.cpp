...
int TripletOptimizer::feed_margin_bpr_loss_with_RotatE_transform(std::vector<double>& from_embedding, std::vector<double>& relation_embedding, std::vector<double>& to_embedding_pos, std::vector<double>& to_embedding_neg, double margin, int dimension, std::vector<double>& from_loss, std::vector<double>& relation_loss, std::vector<double>& to_loss_pos, std::vector<double>& to_loss_neg) {

    std::vector<double> transformed_from_embedding;
    transformed_from_embedding.resize(dimension, 0.0);
    ...
    for (int d=0; d<dimension;d++)
    {
        transformed_from_embedding[d] = from_embedding[d] * relation_embedding[d];
        diff_to_embedding[d] = to_embedding_pos[d] - to_embedding_neg[d];
        prediction += transformed_from_embedding[d] * ( diff_to_embedding[d] );
    }

    gradient = this->fast_sigmoid(0.0-prediction);
    for (int d=0; d<dimension; ++d)
    {
        from_loss[d] += gradient * diff_to_embedding[d] * relation_embedding[d];
        relation_loss[d] += gradient * diff_to_embedding[d] * from_embedding[d];
        to_loss_pos[d] += gradient * transformed_from_embedding[d];
        to_loss_neg[d] -= gradient * transformed_from_embedding[d];
    }
    return 1;
}
