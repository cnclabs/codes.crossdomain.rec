...
print("(TAR / {tar} Train) Before {ncore}core:".format(tar=tar.upper(), ncore=ncore), len(tar_test))
print("(TAR / {tar} Train) After {ncore}core:".format(tar=tar.upper(), ncore=ncore), len(tar_ncore_test))
print("(TAR / {tar} Train) Diff:".format(tar=tar.upper()),len(tar_test)-len(tar_ncore_test))
print("-"*10)
...
print("(SRC / {src} Train) Before {ncore}core:".format(src=src.upper(), ncore=ncore), len(src_test))
print("(SRC / {src} Train) After {ncore}core:".format(src=src.upper(), ncore=ncore), len(src_ncore_test))
print("(SRC / {src} Train) Diff:".format(src=src.upper()), len(src_test)-len(src_ncore_test))
print("-"*10)
print("Finished SRC / {} ...".format(src.upper()))

# make users
print("Start selecting COLD users from SHARED users ...")
shared_users = set(tar_ncore_train.reviewerID).intersection(set(src_ncore_train.reviewerID))
