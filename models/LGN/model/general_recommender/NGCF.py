...
class NGCF(AbstractRecommender):
    ...
    def _create_placeholders(self):
        with tf.compat.v1.name_scope("input_data"):
            ...
            self.user_input = tf.compat.v1.placeholder(tf.int32, shape=(None,))
    ...
    def _create_inference(self):
        with tf.compat.v1.name_scope("inference"):
            ...
            *********************************************************
            ...
            """
            self.u_g_embeddings = tf.nn.embedding_lookup(params=self.ua_embeddings, ids=self.user_input)
    ...
    def _create_loss(self):
        with tf.compat.v1.name_scope("loss"): 
            ...
            self.pos_scores = tf.reduce_sum(input_tensor=tf.multiply(self.u_g_embeddings, self.pos_i_g_embeddings), axis=1)
            neg_scores = tf.reduce_sum(input_tensor=tf.multiply(self.u_g_embeddings, self.neg_i_g_embeddings), axis=1)
            ...
            mf_loss = tf.reduce_sum(input_tensor=maxi)
...
#                                                   tf.reduce_sum(tf.square(self.weights['b_bi_%d' % k])))
    ...
    def _create_ngcf_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            # message dropout.
            ego_embeddings = tf.nn.dropout(ego_embeddings, 1 - (1-self.mess_dropout_ratio))
    ...
    def _create_gcn_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            embeddings = tf.nn.dropout(embeddings, 1 - (1 - self.mess_dropout_ratio))
    ...
    def _create_gcmc_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            mlp_embeddings = tf.nn.dropout(mlp_embeddings, 1 - (1 - self.mess_dropout_ratio))
