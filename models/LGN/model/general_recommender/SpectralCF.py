...
class SpectralCF(AbstractRecommender):
    ...
    def _create_placeholders(self):
        with tf.compat.v1.name_scope("input_data"):
            ...
            self.input_item_pos = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None, ])
    ...
    def _create_inference(self):
        with tf.compat.v1.name_scope("inference"):
            ...
            self.u_embeddings = tf.nn.embedding_lookup(params=self.user_new_embeddings, ids=self.input_user)
            self.pos_i_embeddings = tf.nn.embedding_lookup(params=self.item_new_embeddings, ids=self.input_item_pos)
            self.neg_j_embeddings = tf.nn.embedding_lookup(params=self.item_new_embeddings, ids=self.input_item_neg)
            
    def _create_loss(self):
        with tf.compat.v1.name_scope("loss"): 
            self.output = tf.reduce_sum(input_tensor=tf.multiply(self.u_embeddings, self.pos_i_embeddings), axis=1)
            output_neg = tf.reduce_sum(input_tensor=tf.multiply(self.u_embeddings, self.neg_j_embeddings), axis=1)
