...
class NeuMF(AbstractRecommender):
    ...
    def _create_inference(self, item_input):
        with tf.compat.v1.name_scope("inference"):
            ...
            mf_user_latent = tf.nn.embedding_lookup(params=self.mf_embedding_user, ids=self.user_input)
            mf_item_latent = tf.nn.embedding_lookup(params=self.mf_embedding_item, ids=item_input)
            mlp_user_latent = tf.nn.embedding_lookup(params=self.mlp_embedding_user, ids=self.user_input)
            mlp_item_latent = tf.nn.embedding_lookup(params=self.mlp_embedding_item, ids=item_input)
            ...
            for idx in np.arange(len(self.layers)):
                mlp_vector = tf.compat.v1.layers.dense(mlp_vector, units=self.layers[idx], activation=tf.nn.relu)
    
            # Concatenate MF and MLP parts
            predict = tf.reduce_sum(input_tensor=tf.concat([mf_vector, mlp_vector], axis=1), axis=1)
