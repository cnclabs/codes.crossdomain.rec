...
class ConvNCF(AbstractRecommender):
    ...
    def _create_placeholders(self):
        with tf.compat.v1.name_scope("input_data"):
            ...
            self.keep_prob = tf.compat.v1.placeholder_with_default(1.0, shape=None,name = "keep_prob")
    ...
    def _conv_layer(self, x, P):
        conv = tf.nn.conv2d(input=x, filters=P[0], strides=[1, 2, 2, 1], padding='SAME')
    ...
    def _create_inference(self, item_input):
        with tf.compat.v1.name_scope("inference"):
            ...
            self.embedding_q = tf.nn.embedding_lookup(params=self.embedding_Q, ids=item_input)
            ...
            for p in self.P:
                ...
                input = self.layer[-1]
            # prediction
            self.dropout = tf.nn.dropout(self.layer[-1], 1 - (self.keep_prob))
    ...
    def _create_pre_optimizer(self):
        self.pre_opt = tf.compat.v1.train.AdagradOptimizer(learning_rate=0.01).minimize(self.loss)
    ...
    def _create_optimizer(self):
        with tf.compat.v1.name_scope("learner"):
            ...
            var_list2 = list(set(tf.compat.v1.trainable_variables()) - set(var_list1))
            opt1 = tf.compat.v1.train.AdagradOptimizer(self.lr_embed)
            opt2 = tf.compat.v1.train.AdagradOptimizer(self.lr_net)
            grads = tf.gradients(ys=self.opt_loss, xs=var_list1 + var_list2)
