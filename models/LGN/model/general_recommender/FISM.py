...
class FISM(AbstractRecommender):
    ...
    def _create_inference(self, user_input, item_input, num_idx):
        with tf.compat.v1.name_scope("inference"):
            embedding_p = tf.reduce_sum(input_tensor=tf.nn.embedding_lookup(params=self.embedding_Q_, ids=user_input), axis=1)
            ...
            output = coeff * tf.reduce_sum(input_tensor=tf.multiply(embedding_p,embedding_q), axis=1) + bias_i
