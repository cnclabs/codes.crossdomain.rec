...
class DiffNet(SocialAbstractRecommender):
    ...
    def _create_placeholders(self):
        with tf.compat.v1.name_scope("input_data"):
            ...
            self.labels = tf.compat.v1.placeholder(tf.float32, shape=[None], name="labels_input")
    ...
    def _create_variables(self):
        ...
        self.reduce_dimension_layer = tf.compat.v1.layers.Dense(self.embedding_size, activation=tf.nn.sigmoid,
                                                      name='reduce_dimension_layer')
        
        self.item_fusion_layer = tf.compat.v1.layers.Dense(self.embedding_size, activation=tf.nn.sigmoid,
                                                 name='item_fusion_layer')
        self.user_fusion_layer = tf.compat.v1.layers.Dense(self.embedding_size, activation=tf.nn.sigmoid,
    ...
    def generateUserEmbeddingFromSocialNeighbors(self, current_user_embedding):
        user_embedding_from_social_neighbors = tf.sparse.sparse_dense_matmul(self.social_neighbors_sparse_matrix,
    ...
    def generateUserEmebddingFromConsumedItems(self, current_item_embedding):
        user_embedding_from_consumed_items = tf.sparse.sparse_dense_matmul(self.consumed_items_sparse_matrix,
    ...
    def _create_inference(self):
        with tf.compat.v1.name_scope("inference"):
            ...
            # embedding look up
            self.latest_user_latent = tf.nn.embedding_lookup(params=self.final_user_embedding, ids=self.user_input)
            self.latest_item_latent = tf.nn.embedding_lookup(params=self.final_item_embedding, ids=self.item_input)
            
#             self.output = tf.sigmoid(tf.reduce_sum(tf.multiply(self.latest_user_latent, self.latest_item_latent),1))
            self.output = tf.reduce_sum(input_tensor=tf.multiply(self.latest_user_latent, self.latest_item_latent), axis=1)
    ...
    def _create_loss(self):
        with tf.compat.v1.name_scope("loss"):
            ...
            self.loss = tf.compat.v1.losses.sigmoid_cross_entropy(self.labels, self.output) + \
