...
class Fossil(SeqAbstractRecommender):
    ...
    def _create_placeholders(self):
        with tf.compat.v1.name_scope("input_data"):
            self.user_input_id = tf.compat.v1.placeholder(tf.int32, shape=[None, ], name="user_input_id")  # the index of users
    ...
    def _create_inference(self, user_input, item_input, num_idx):
        with tf.compat.v1.name_scope("inference"):
            embedding_p = tf.reduce_sum(input_tensor=tf.nn.embedding_lookup(params=self.embedding_P, ids=user_input), axis=1)
            embedding_eta_u = tf.nn.embedding_lookup(params=self.eta, ids=self.user_input_id)
            batch_size = tf.shape(input=embedding_eta_u)[0]
            eta = tf.expand_dims(tf.tile(self.eta_bias, tf.stack([batch_size, 1])) + embedding_eta_u, -1)
            embeddings_short = tf.nn.embedding_lookup(params=self.embedding_P, ids=self.item_input_recent)
            embedding_q = tf.nn.embedding_lookup(params=self.embedding_Q, ids=item_input)
            bias_i = tf.nn.embedding_lookup(params=self.bias, ids=item_input)
            coeff = tf.pow(num_idx, -tf.constant(self.alpha, tf.float32, [1]))
            output = coeff * tf.reduce_sum(input_tensor=tf.multiply(embedding_p, embedding_q), axis=1) +\
                     tf.reduce_sum(input_tensor=tf.multiply(tf.reduce_sum(input_tensor=eta*embeddings_short, axis=1), embedding_q), axis=1) + bias_i
