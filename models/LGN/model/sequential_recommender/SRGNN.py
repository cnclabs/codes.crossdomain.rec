...
class SRGNN(SeqAbstractRecommender):
    ...
    def _create_variable(self):
        self.mask_ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[self.batch_size, None])
        self.alias_ph = tf.compat.v1.placeholder(dtype=tf.int32, shape=[self.batch_size, None])  # 给给每个输入重新
        self.item_ph = tf.compat.v1.placeholder(dtype=tf.int32, shape=[self.batch_size, None])  # 原始ID+padID, pad 在后面
        self.target_ph = tf.compat.v1.placeholder(dtype=tf.int32, shape=[self.batch_size])

        self.adj_in_ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[self.batch_size, None, None])
        self.adj_out_ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[self.batch_size, None, None])

        stdv = 1.0 / math.sqrt(self.hidden_size)
        w_init = tf.compat.v1.random_uniform_initializer(-stdv, stdv)
        self.nasr_w1 = tf.compat.v1.get_variable('nasr_w1', [self.hidden_size, self.hidden_size], dtype=tf.float32,
                                       initializer=w_init)
        self.nasr_w2 = tf.compat.v1.get_variable('nasr_w2', [self.hidden_size, self.hidden_size], dtype=tf.float32,
                                       initializer=w_init)
        self.nasr_v = tf.compat.v1.get_variable('nasrv', [1, self.hidden_size], dtype=tf.float32, initializer=w_init)
        self.nasr_b = tf.compat.v1.get_variable('nasr_b', [self.hidden_size], dtype=tf.float32,
                                      initializer=tf.compat.v1.zeros_initializer())

        embedding = tf.compat.v1.get_variable(shape=[self.num_item, self.hidden_size], name='embedding',
        ...
        self.W_in = tf.compat.v1.get_variable('W_in', shape=[self.hidden_size, self.hidden_size], dtype=tf.float32,
                                    initializer=w_init)
        self.b_in = tf.compat.v1.get_variable('b_in', [self.hidden_size], dtype=tf.float32, initializer=w_init)
        self.W_out = tf.compat.v1.get_variable('W_out', [self.hidden_size, self.hidden_size], dtype=tf.float32,
                                     initializer=w_init)
        self.b_out = tf.compat.v1.get_variable('b_out', [self.hidden_size], dtype=tf.float32, initializer=w_init)

        self.B = tf.compat.v1.get_variable('B', [2 * self.hidden_size, self.hidden_size], initializer=w_init)

    def ggnn(self):
        fin_state = tf.nn.embedding_lookup(params=self.embedding, ids=self.item_ph)  # (b,l,d)
        cell = tf.compat.v1.nn.rnn_cell.GRUCell(self.hidden_size)
        with tf.compat.v1.variable_scope('gru'):
            for i in range(self.step):
                ...
                state_output, fin_state = tf.compat.v1.nn.dynamic_rnn(cell, av, initial_state=fin_state_tmp)
        return tf.reshape(fin_state, [self.batch_size, -1, self.hidden_size])  # (b,l,d)

    def _session_embedding(self, re_embedding):
        # re_embedding  (b,l,d)
        rm = tf.reduce_sum(input_tensor=self.mask_ph, axis=1)  # (b,), length of each session
        last_idx = tf.stack([tf.range(self.batch_size), tf.cast(rm, dtype=tf.int32) - 1], axis=1)  # (b, 2) index of last item
        ...
        seq_h = [tf.nn.embedding_lookup(params=re_embedding[i], ids=self.alias_ph[i]) for i in range(self.batch_size)]
        ...
        if not self.nonhybrid:
            ...
            sess_embedding = tf.reduce_sum(input_tensor=tf.reshape(coef, [self.batch_size, -1, 1]) * seq_h, axis=1)
    ...
    def build_graph(self):
        ...
        learning_rate = tf.compat.v1.train.exponential_decay(self.lr, global_step=global_step, decay_steps=decay,
                                                   decay_rate=self.lr_dc, staircase=True)
        self.train_opt = tf.compat.v1.train.AdamOptimizer(learning_rate).minimize(loss_train, global_step=global_step)
