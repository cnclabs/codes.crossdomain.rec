...
class GRU4RecPlus(SeqAbstractRecommender):
    ...
    def _create_variable(self):
        self.X_ph = tf.compat.v1.placeholder(tf.int32, [self.batch_size], name='input')
        self.Y_ph = tf.compat.v1.placeholder(tf.int32, [self.batch_size+self.n_sample], name='output')
        self.state_ph = [tf.compat.v1.placeholder(tf.float32, [self.batch_size, n_unit], name='layer_%d_state' % idx)
    ...
    def _softmax_neg(self, logits):
        # logits: (b, size_y)
        hm = 1.0 - tf.eye(tf.shape(input=logits)[0], tf.shape(input=logits)[1])
        logits = logits * hm
        logits = logits - tf.reduce_max(input_tensor=logits, axis=1, keepdims=True)
        e_x = tf.exp(logits) * hm  # (b, size_y)
        e_x = e_x / tf.reduce_sum(input_tensor=e_x, axis=1, keepdims=True)
        return e_x  # (b, size_y)

    def _bpr_max_loss(self, logits):
        # logits: (b, size_y)
        softmax_scores = self._softmax_neg(logits)  # (b, size_y)
        pos_logits = tf.linalg.diag_part(logits)  # (b,)
        pos_logits = tf.reshape(pos_logits, shape=[-1, 1])  # (b, 1)
        prob = tf.sigmoid((pos_logits - logits))  # (b, size_y)
        prob = tf.reduce_sum(input_tensor=tf.multiply(prob, softmax_scores), axis=1)  # (b,)
        loss = -tf.math.log(prob + 1e-24)
        reg_loss = tf.reduce_sum(input_tensor=tf.multiply(tf.pow(logits, 2), softmax_scores), axis=1)  # (b,)

        return tf.reduce_mean(input_tensor=loss + self.bpr_reg*reg_loss)
    ...
    def _top1_max_loss(self, logits):
        ...
        loss = tf.reduce_sum(input_tensor=tf.multiply(prob, softmax_scores), axis=1)
    ...
    def build_graph(self):
        ...
        cells = [tf.compat.v1.nn.rnn_cell.GRUCell(size, activation=self.hidden_act) for size in self.layers]
        drop_cell = [tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell) for cell in cells]
        stacked_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell(drop_cell)
        inputs = tf.nn.embedding_lookup(params=self.input_embeddings, ids=self.X_ph)  # (b, l1)
        ...
        # for training
        items_embed = tf.nn.embedding_lookup(params=self.item_embeddings, ids=self.Y_ph)  # (size_y, ln)
        ...
        self.update_opt = tf.compat.v1.train.AdamOptimizer(self.lr).minimize(final_loss)
