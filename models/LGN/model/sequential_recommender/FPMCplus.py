...
class FPMCplus(SeqAbstractRecommender):
    ...
    def _attention_mlp(self, embeddings_UI_u, embeddings_IL_i, item_embedding_recent):
        with tf.compat.v1.name_scope("attention_MLP"):
            ...
            batch_users = tf.shape(input=embedding_short)[0]
            ...
            return tf.reduce_sum(input_tensor=A * item_embedding_recent, axis=1)    
            
    def _create_inference(self, item_input):
        with tf.compat.v1.name_scope("inference"):
            # embedding look up
            embeddings_UI_u = tf.nn.embedding_lookup(params=self.embeddings_UI, ids=self.user_input)
            embeddings_IU_i = tf.nn.embedding_lookup(params=self.embeddings_IU, ids=item_input)
            embeddings_IL_i = tf.nn.embedding_lookup(params=self.embeddings_IL, ids=item_input)
            embeddings_LI_l = tf.nn.embedding_lookup(params=self.embeddings_LI, ids=self.item_input_recent)
            ...
            predict = tf.reduce_sum(input_tensor=predict_vector, axis=1)
