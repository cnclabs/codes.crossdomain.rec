...
class Caser(SeqAbstractRecommender):
    ...
    def _create_variable(self):
        ...
        l2_regularizer = tf.keras.regularizers.l2(0.5 * (self.l2_reg))

        self.user_embeddings = tf.compat.v1.get_variable('user_embeddings', dtype=tf.float32,
                                               shape=[self.users_num, self.factors_num],
                                               regularizer=l2_regularizer)

        item_embeddings = tf.compat.v1.get_variable('seq_item_embeddings', dtype=tf.float32,
        ...
        self.conv_v = tf.compat.v1.layers.Conv2D(self.nv, [self.seq_L, 1])
        self.conv_h = [tf.compat.v1.layers.Conv2D(self.nh, [i, self.factors_num], activation="relu")
                       for i in np.arange(1, self.seq_L + 1)]

        self.fc1_ly = tf.compat.v1.layers.Dense(self.factors_num, activation="relu")
        self.dropout_ly = tf.compat.v1.layers.Dropout(self.dropout)

        # predication embedding
        self.item_embeddings = tf.compat.v1.get_variable('item_embeddings', dtype=tf.float32,
                                               shape=[self.items_num, self.factors_num * 2],
                                               regularizer=l2_regularizer)
        self.item_biases = tf.compat.v1.get_variable('item_biases', dtype=tf.float32, shape=[self.items_num],
                                           initializer=tf.compat.v1.initializers.zeros(), regularizer=l2_regularizer)

    def build_graph(self):
        self._create_variable()
        # embedding lookup
        batch_size = tf.shape(input=self.item_seq_ph)[0]
        item_embs = tf.nn.embedding_lookup(params=self.seq_item_embeddings, ids=self.item_seq_ph)  # (b, L, d)
        ...
        for conv_h in self.conv_h:
            ...
            pool_out = tf.reduce_max(input_tensor=conv_out, axis=1)  # (b, nh)
        ...
        # rating calculation
        user_embs = tf.nn.embedding_lookup(params=self.user_embeddings, ids=self.user_ph)  # (b, d)
        ...
        tar_item_embs = tf.nn.embedding_lookup(params=self.item_embeddings, ids=tar_item)  # (b, 2T, 2d)
        ...
        # loss
        pos_loss = tf.reduce_mean(input_tensor=-tf.math.log(tf.sigmoid(pos_logits) + 1e-24))
        neg_loss = tf.reduce_mean(input_tensor=-tf.math.log(1 - tf.sigmoid(neg_logits) + 1e-24))
        ...
        self.train_opt = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(loss)
