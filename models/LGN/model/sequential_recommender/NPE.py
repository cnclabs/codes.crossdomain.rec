...
class NPE(SeqAbstractRecommender):
    ...
    def _create_inference(self):
        with tf.compat.v1.name_scope("inference"):
            ...
            embeddings_IU_i = tf.nn.embedding_lookup(params=self.embeddings_IU,ids=self.item_input)
            embeddings_LI_l = tf.nn.embedding_lookup(params=self.embeddings_IL, ids=self.item_input_recent)
            
            context_embedding = tf.reduce_sum(input_tensor=embeddings_LI_l, axis=1)
