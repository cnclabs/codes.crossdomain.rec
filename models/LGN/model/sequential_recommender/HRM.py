...
class HRM(SeqAbstractRecommender):
    ...
    def avg_pooling(self, input_embedding):
        output = tf.reduce_mean(input_tensor=input_embedding, axis=1)
        return output

    def max_pooling(self, input_embedding):
        output = tf.reduce_max(input_tensor=input_embedding, axis=1)
        return output
    ...
    def _create_inference(self):
        with tf.compat.v1.name_scope("inference"):
            ...
            item_embedding = tf.nn.embedding_lookup(params=self.item_embeddings, ids=self.item_input)
