...
with open('./input/csj_lightfm.pickle', 'rb') as pickle_file:
  book = pickle.load(pickle_file)
with open('./input/hk_lightfm.pickle', 'rb') as pickle_file:
...
hk_test_df = hk_test_df[['reviewerID', 'asin']]
hk_test_df.columns = ['user_id', 'item_id']
hk_test_df['user_id'] = hk_test_df['user_id'].apply(lambda x: 'user_'+x)

## sample testing users
sample_amount = 4000
random.seed(3)
if args.test_users == 'target':
  testing_users = random.sample(set(hk_test_df.user_id), sample_amount)
...
# rec pool
## load hk_train_df
hk_train_df = pd.read_csv('../../CPR/input/hk_train_input.txt', header=None, sep='\t')
hk_train_df.columns = ['user_id', 'item_id', 'rating']
total_item_set = set(hk_train_df.item_id)
...
def process_user_rec_dict(user_list):
  ...
  for user in user_list:
      watched_set = set(hk_train_df[hk_train_df['user_id'] == user].item_id)
      neg_pool = total_item_set - watched_set
      random.seed(5)
      neg_99 = random.sample(neg_pool, 99)
      user_rec_pool = list(neg_99) + list(hk_test_df[hk_test_df['user_id'] == user].item_id)
...
with open('./input/hk_item_emb_dict.pickle', 'rb') as pickle_file:
    rec_pool_emb_dict = pickle.load(pickle_file)

# read hk domain emb
with open('./input/hk_emb_dict.pickle', 'rb') as pickle_file:
    hk_emb_dict = pickle.load(pickle_file)
  ...
  for user in user_list:
    ...
    for item in user_rec_pool_dict[user]:
      ...
      if modelB.config['use_cuda'] is True:
        score = modelB(Variable(torch.FloatTensor(hk_emb_dict[user])).cuda(),\
                       Variable(torch.FloatTensor(rec_pool_emb_dict[item])).cuda())
      else:
        score = modelB(Variable(torch.FloatTensor(hk_emb_dict[user])),\
...
for epoch in range(config['num_epoch']):
    ...
    for user in testing_users:
      count += 1
      # ground truth
      test_data = list(hk_test_df[hk_test_df['user_id']==user]['item_id'])
    ...
    print('[HK Evluating Epoch {}], Recall@10 = {}, NDCG@10 = {}'.format(epoch, str(recall_10), str(ndcg_10)))
    engine.save('./model_status/csj_hk/' + args.test_users + '_' + 'epoch_' + str(epoch) +'_model_')

    with open('./result/DDTCDR_amazon_hk_lightfm' + '_' + args.test_users + '_' + 'result.txt', 'a') as fw:
        fw.writelines(['=================================\n',
                'epoch = ', str(epoch),
