...
with open('./input/tv_lightfm.pickle', 'rb') as pickle_file:
  book = pickle.load(pickle_file)
with open('./input/vod_lightfm.pickle', 'rb') as pickle_file:
...
# ground truth 
with open('../../LOO_data/vod_test_df.pickle', 'rb') as pf:
...
# rec pool
## load vod_train_df
vod_train_df = pd.read_csv('../../CPR/input/vod_train_input.txt', header=None, sep='\t')
vod_train_df.columns = ['user_id', 'item_id', 'rating']
...
for user in testing_users:
    ...
    user_rec_pool_dict[user] = user_rec_pool
...
with open('./input/vod_item_emb_dict.pickle', 'rb') as pickle_file:
    rec_pool_emb_dict = pickle.load(pickle_file)

# read vod domain emb
with open('./input/vod_emb_dict.pickle', 'rb') as pickle_file:
    vod_emb_dict = pickle.load(pickle_file)
  ...
  for user in user_list:
    ...
    for item in user_rec_pool_dict[user]:
      ...
      if modelB.config['use_cuda'] is True:
        score = modelB(Variable(torch.FloatTensor(vod_emb_dict[user])).cuda(),\
                       Variable(torch.FloatTensor(rec_pool_emb_dict[item])).cuda())
      else:
        score = modelB(Variable(torch.FloatTensor(vod_emb_dict[user])),\
...
for epoch in range(config['num_epoch']):
    ...
    print('[VOD Evluating Epoch {}], Recall@10 = {}, NDCG@10 = {}'.format(epoch, str(recall_10), str(ndcg_10)))
    engine.save('./model_status/tv_vod/' + args.test_users + '_'+ 'epoch_' + str(epoch) +'_model_')

    with open('./result/DDTCDR_kk_vod_lightfm' + '_' + args.test_users + '_' + 'result.txt' , 'a') as fw:
        fw.writelines(['=================================\n',
                'epoch = ', str(epoch),
