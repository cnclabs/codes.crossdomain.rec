...
vod_ratings = vod_train_df.groupby(['user_id', 'item_id']).size().apply(lambda x: log(x+1.))
...
with open('../../user/tv_vod_shared_users.pickle', 'rb') as pk:
    shared_users = pickle.load(pk)

# extract tv domain embedding of shared_users as user_info
shared_users_emb = {}
...
# sort vod_ratings, putting shared_users' logs to the upper part 
shared_users_vod_ratings = vod_ratings[vod_ratings['UserId'].isin(shared_users)]
non_shared_users_vod_ratings = vod_ratings[~vod_ratings['UserId'].isin(shared_users)]
sorted_vod_ratings = pd.concat([shared_users_vod_ratings, non_shared_users_vod_ratings], ignore_index=True)
...
if args.test_users == 'target':
  testing_users = random.sample(vod_test_df.user_id, sample_amount)
...
for user in testing_users:
    ...
    count += 1
    if count%100 == 0:
        print("finished {} users rec dict".format(count))
