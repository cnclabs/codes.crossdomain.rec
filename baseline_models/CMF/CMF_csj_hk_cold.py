...
# open csj hk cold start users
with open('../../user/csj_hk_cold_users.pickle', 'rb') as pf:
    csj_hk_cold_start_users = pickle.load(pf)

filtered_hk_train = hk_train[~hk_train['reviewerID'].isin(csj_hk_cold_start_users)]
filtered_hk_train = filtered_hk_train[['reviewerID','asin','overall']]
...
## extract csj domain embedding of overlap_users as user_info
...
overlap_users = set(csj_train.reviewerID).intersection(set(filtered_hk_train.reviewerID))
...
item_emb_dict = {}
with open('../BPR/graph/cold_hk_lightfm_bpr_10e-5.txt', 'r') as f:
    skip_f = f.readlines()[1:]
    for line in skip_f:
        line = line[:-1]
        prefix = line.split(' ')[0]
...
# sort hk_ratings 
overlap_users_hk_ratings = hk_ratings[hk_ratings['UserId'].isin(overlap_users)]
non_overlap_users_hk_ratings = hk_ratings[~hk_ratings['UserId'].isin(overlap_users)]
sorted_hk_ratings = pd.concat([overlap_users_hk_ratings, non_overlap_users_hk_ratings], ignore_index=True)
...
testing_users = csj_hk_cold_start_users
...
# get cold-start users embedding from csj domain
