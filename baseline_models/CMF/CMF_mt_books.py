...
filtered_books_train = books_train[['reviewerID','asin','overall']]
...
books_ratings['UserId'] = books_ratings['UserId'].apply(lambda x: 'user_'+x)
print("Finished generating books_ratings...")

total_item_set = set(books_ratings.ItemId)

with open('../../user/mt_books_shared_users.pickle', 'rb') as pf:
    shared_users = pickle.load(pf)

# get user_info
# extract mt domain embedding of shared_users as user_info
shared_users_emb = {}
...
item_emb_dict = {}
with open('../BPR/graph/books_lightfm_bpr_10e-5.txt', 'r') as f:
    for line in f:
        # for lightfm bpr emb
        ## remove '\n'
        line = line[:-1]
...
# sort books_ratings, putting shared_users' logs to the upper part 
shared_users_books_ratings = books_ratings[books_ratings['UserId'].isin(shared_users)]
non_shared_users_books_ratings = books_ratings[~books_ratings['UserId'].isin(shared_users)]
sorted_books_ratings = pd.concat([shared_users_books_ratings, non_shared_users_books_ratings], ignore_index=True)
...
if args.test_users == 'target':
  testing_users = random.sample(set(books_test_df.reviewerID), sample_amount)
